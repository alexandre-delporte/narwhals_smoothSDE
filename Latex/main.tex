...
1.8$ for the construction of tensor splines basis function. We choose $\sigma_{\tau}=0.1$ h and $\sigma_{\nu}=0.3$ km/h to represent the individual variability, wheras $\tau_0$ and $\nu_0$ are fixed respectively to $1$ h and $4$ km/h. The values of the spline coefficients $\omega_k$, $k \in \{0,\cdots,L\}$ are given in Table~\ref{table: estimation performance rect}. For each trajectory, initial velocity is set to $0$ and initial position is uniformly sampled in the rectangular domain, at least $500$ metres away from the boundary. To simulate the trajectories, we suppose that the value of the parameter $\omega$ is constant on each time step, that is equal to 
\begin{equation}
...
100 batches of 6 individual trajectories over $24$ hours were also simulated in Scoresby Sound fjords system in which the narwhals move under constraints in our application. The parameters were set as in~\ref{eq: rect simulation parameters}, with only the coefficients $\omega_k$, $k \in \{1,\cdots,L\}$ changed to adapt to this specific domain. The exact values of the spline coefficients can be found in Table~\ref{table: estimation performance fjords}.
Again, for each trajectory, initial velocity was set to $0$ and initial position is uniformly sampled within the sea, with the requirement that it should be at least $500$ metres away from the shore. Time steps are also kept constant, equal to $1$ min. None of the $600$ trajectories reached land. Note that if time steps are too large, the assumption of constant covariates between observations is very likely to be violated due to the steepness of the shoreline, resulting in samples that reach land very often. For the fjords system, this already happens if we try to sample directly with $5$ min time steps. Moreover, sampling within the fjords takes much more time than within the rectangular domain, since it is necessary to loop over each polygon of the geometry to compute the nearest shore point for each new position. 
...
For each batch of $6$ trajectories, we obtain estimates $\widehat{\log(\tau_0)}^{(k)}$, $\widehat{\log(\nu_0)}^{(k)}$, $\hat{\omega}_l^{(k)}, l \in \{0,\cdots, L\}$, $\hat{\sigma}_{\tau}^{(k)}$ and $\hat{\sigma}_{\nu}^{(k)}$, $k \in \{1,\cdots,100\}$ and compute the root mean squared error and the absolute biais of the estimator. The results are summarized in Tables~\ref{table: estimation performance rect} and~\ref{table: estimation performance fjords}.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|}
		...
		\hline
		$\log(\tau_0)$  & 0& 0.24 &  0.26\\
		\hline
		$\log(\nu_0)$  & 1.38 & 0.03 & 0.04 \\
		\hline
		$\omega_0$  &  -0.19  & 0.80 & 0.82 \\
		\hline
		$\omega_1$  & 16.68 & 11.54 & 12.32 \\
		\hline
		$\omega_2$  & -3.58 & 2.58 &  3.04\\
		\hline
		$\omega_3$  & 13.07 &  8.74 &  8.84\\
		\hline
		$\omega_4$  & 42.11 & 26.36 &  29.04\\
		\hline
		$\omega_5$  & 13.89 & 8.44 & 8.54 \\
		\hline
		$\omega_6$  &  61.53 & 51.45 &  51.60\\
		\hline
		$\omega_7$  & 10.15 & 2.54 & 3.37 \\
		\hline
		$\omega_8$  &  12.31 & 4.22 & 4.45 \\
		\hline
		$\sigma_{\tau}$ & 0.1 & & \\
		\hline
		$\sigma_{\nu}$ & 0.3 & & \\
...
\begin{equation}  \forall i \in \{1,\cdots,N\}, \quad \left\{
	...
	f_{\omega}(E^{shore},\Theta)=\omega_{0,pre}+\sum_{k=1}^{L} \omega_{k,pre} \psi_k(E^{shore},\Theta) + b_{\omega,pre}^{(i)}, \quad (E^{shore},\Theta) \in \R^+ \times [-\pi,\pi]
	\end{array}
	\right.
...
The $\psi_k$ in equation~\ref{eq: exact baseline model} are  known bivariate cubic splines basis functions, constructed through tensor products from the univariate basis functions as described in \cite{wood_generalized_2017}, section 4.1.8.
...
\begin{equation}  \forall i \in \{1,\cdots,N\}, \quad \left\{
	\begin{array}{l}
		...
		\omega_i(t)=f_{\omega}(E^{shore}_i(t),\Theta_i(t))\\
		f_{\omega}(E^{shore},\Theta)=\omega_{0,post}+\sum_{k=1}^{L} \omega_{k,post} \psi_k(E^{shore},\Theta) + b_{\omega,post}^{(i)}, \quad (E^{shore},\Theta) \in \R^+ \times [-\pi,\pi]
	\end{array}
	\right.
...
Again, $\log(\tau_{0,post})$, $\log(\nu_{0,post})$, $\omega_{0,post}$ are population intercepts and $b^{(i)}_{post} \sim \mathcal{N}\left( 0, \diag\left(\frac{1}{\lambda_{post}}\right)\right)$ with $\lambda_{post}=\begin{pmatrix} \frac{1}{\sigma_{\tau,post}^2} & \frac{1}{\sigma_{\nu,post}^2} & \frac{1}{\sigma_{\omega,post}^2} \end{pmatrix}^\top$ and $q_{\tau}$ and $q_{\nu}$ are the spline degrees of freedom for the exposure to the ship covariate.
