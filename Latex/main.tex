...
\begin{figure}[H]
	...
	\begin{subfigure}{0.48\textwidth}
		\centering
		\includegraphics[scale=0.3]{images/simulation study/illustrative_sample_rect.png}
		\caption{}
	\end{subfigure}
	\begin{subfigure}{0.48\textwidth}
		\centering
		\includegraphics[scale=0.3]{images/simulation study/illustrative_sample_fjords.png}
		\caption{}
	\end{subfigure}
...
\subsection{Estimation of angular velocity $\omega$ with tensor splines}
...
\begin{equation}  \left\{
	...
	\log(\nu_i)=\log(\nu_{pre})+b_{\nu,pre}^{(i)}  \\
	\omega_i=\omega_{0,pre}+\sum_{k=1}^{L} \omega_{k,pre} \psi_k(E_{shore}^{(i)}(t_j^{(i)}),\Theta^{(i)}(t_j^{(i)})) + b_{\omega,pre}^{(i)}
	\end{array}
	\right.
\end{equation}
