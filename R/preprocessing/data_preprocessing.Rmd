...
```{r define ExpShipI and ExpShipT}
...
\item  Project to UTM zone 26. The narwhal positions are in latitude and longitude.To analyze the track and visualize it better we project it on a plane in UTM coordinastes.
...
# Time steps distribution during exposure vs no exposure 
...
```{r save points as geojson in data folder}
points_sf <- st_as_sf(dataprep[,c("x","y")], coords = c("x","y"))
st_crs(points_sf)="+init=EPSG:32626 +units=km"
st_write(points_sf,file.path(narwhal_data_path,"narwhals_positions.shp"))
```
...
#get land polygons
land_utm<-st_read(file.path(greenland_data_path,"land_utm.shp"))

#get land polygons
modified_land_utm<-st_read(file.path(greenland_data_path,"modified_land_utm.shp"))
modified_land_utm <- st_transform(modified_land_utm, crs = "+init=EPSG:32626 +units=km")

#scoresby sound google
land_google_utm<-st_read(file.path(greenland_data_path,"land_google_utm.shp"))
st_crs(land_google_utm)="+init=EPSG:32626"
```

## Plot coastline and land

```{r plot coastline and land}
plot_land=ggplot()+geom_sf(data=land_utm$geometry,fill="grey",col="brown")+
  coord_sf(datum=st_crs(32626))
plot_modified_land=ggplot()+geom_sf(data=modified_land_utm$geometry,fill="grey",col="blue")+
  coord_sf(datum=st_crs(32626))
plot_land_google=ggplot()+geom_sf(data=land_google_utm$geometry,fill="grey",col="blue")+
  coord_sf(datum=st_crs(32626))
```
...
ggplotly(plot_modified_land)
```

```{r}
ggplotly(plot_land_google)
```
...
For the rest of the analysis, we'll keep the bounded domain defined by the polygons in \texttt{modified\_land\_utm} sf object as a reference.
...
cat("Percentage of points on land with distance from modified : \n",
    length(which(res_modified_land$min_distances==0))/length(dataprep$time)*100)
```
...
check_plot1=ggplot()+geom_sf(data=land_google_utm$geometry,fill="grey")+
    coord_sf(datum=st_crs(32626))+
  geom_point(data=dataprep,aes(x,y,col=(DistanceShore<0.05)),size=0.2)+
...
check_plot2=ggplot()+geom_sf(data=land_utm$geometry,fill="grey")+
    coord_sf(datum=st_crs(32626))+
...
df=data.frame(DistanceShore=res_land$min_distances[res_land$min_distances<0.2 & res_land$min_distances>0  ])
...
This seems to be pretty uniformly distributed. It is difficult to assess at which distance the narwhals stay from 
the shore in reality.
Let's choose arbitrarily a threshold of 50 metres.
...
ggsave(path=plots_folder,"realExpShore_through_time.png", plot=pExpShore)
```
...
pBE1=milne_land_map + 
  geom_point(dataBE1[,c("ID","Long","Lat","time")],
             mapping=aes(Long, Lat, col = ID),size= 0.5,alpha=1) +
   geom_path(dataBE1[,c("ID","Long","Lat","time")],mapping=aes(Long,Lat,col=ID),linewidth=0.1)+
  geom_point(data = dataBE1[,c("ID","Long","Lat","time")] %>% filter(!duplicated(ID)),
             aes(x = Long, y = Lat,col=ID), shape = 3, size = 4,col = "red")+
  scale_color_viridis_d(labels=c("A1","A2","A3","A4","A5","A6"))

pBE2=milne_land_map + 
  geom_point(dataBE2[,c("ID","Long","Lat","time")],
             mapping=aes(Long, Lat, col = ID),size= 0.5,alpha=1) +
   geom_path(dataBE2[,c("ID","Long","Lat","time")],mapping=aes(Long,Lat,col=ID),linewidth=0.1)+
  geom_point(data = dataBE2[,c("ID","Long","Lat","time")] %>% filter(!duplicated(ID)),
             aes(x = Long, y = Lat,col=ID), shape = 3, size = 4,col = "red")+
  scale_color_viridis_d(labels=c("A1","A2","A3","A4","A5","A6"))
