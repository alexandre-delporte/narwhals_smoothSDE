...
coastline_utm<-st_read(file.path(greenland_data_path,"coastline_utm.geojson"))
...
plot(res_coastline$min_distances-dataprep$DistanceShore)
```
...
df=data.frame(DistanceShore=res_coastline$min_distances[res_coastline$min_distances>0])
histo2=ggplot(df, aes(x =DistanceShore)) +
  geom_histogram(fill="cyan",bins=100,color="black")+xlab("Computed distance from coastline (km)")
...
cat("Percentage of points on land with distance from coastline : \n",
    length(which(res_coastline$min_distances==0))/length(dataprep$time)*100)
```
...
df=data.frame(DistanceShore=res_land$min_distances[res_land$min_distances<2 & res_land$min_distances>0  ])
