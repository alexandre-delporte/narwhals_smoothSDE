...
st_write(points_sf,file.path(narwhal_data_path,"narwhals_positions.shp"),append=FALSE)
```
...
quantile(res_land$min_distance,probs=c(0.05,0.95))
D_low=quantile(res_land$min_distance,probs=0.05)
D_up=median(res_land$min_distance)
```
...
check_plot1=ggplot()+geom_sf(data=land_utm$geometry,fill="grey")+
    coord_sf(datum=st_crs(32626))+
...
df=data.frame(DistanceShore=res_land$min_distances[res_land$min_distances<1 & res_land$min_distances>0  ])
...
for (id in unique(dataprep$ID)) {
  sub_data=dataprep[dataprep$ID==id,]
  id_track=ggplot()+geom_sf(data=land_utm$geometry,fill="grey")+
    coord_sf(datum=st_crs(32626))+
...
```{r}
df=dataprep
df[df$DistanceShore<D_low,"ExpShore"]=1/D_low
df[df$DistanceShore>D_up,"ExpShore"]=0

pExpShore=ggplot(df,aes(time,ExpShore,col=ID))+ geom_line(color="black")+facet_grid(ID ~.,labeller=as_labeller(c("Asgeir"="A1","Frederik"="A2","Helge18"="A3","Kyrri"="A4","Nemo"="A5","Siggi"="A6")))+
...
for (id in unique(data$ID)) {
  narwhalDataBE=dataBE2[dataBE2$ID==id,]
