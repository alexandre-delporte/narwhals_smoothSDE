...
animate_map <- milne_land_map + 
  geom_point(data = dataprep[,c("ID","Long","Lat","time")], 
             mapping = aes(Long, Lat, shape = ID, col = ID), 
             size = 4, alpha = 1) +
  geom_path(data = dataprep[,c("ID","Long","Lat","time")], 
            mapping = aes(Long, Lat, col = ID), 
            size = 1, alpha = 0.5) +
  scale_color_viridis_d(name = "ID",  # Give a common legend title
                        labels = c("A1","A2","A3","A4","A5","A6")) + 
  scale_shape_manual(name = "ID",  # Same name as color to merge legends
                     values = c(0, 1, 2, 3, 4, 5),  
                     labels = c("A1","A2","A3","A4","A5","A6")) +  
  guides(color = "legend", shape = "legend") +  # Ensure single legend
...
animation=animate(animate_map,nframes = 100, fps = 5,renderer = gifski_renderer())
...
```{=tex}
A lot of points are indeed still very close to land. We think that this means the shoreline is still not accurate.
To solve the problem, we translate the polygon vertices 100m, 200m and 300m inward. 
We compute new distances to shore based on the translated polygon.

```

## Compute distances from shrunk land polygons

```{r get shrunk polygons}

#get shrunk land polygons
shrunk100_scoresby_sound_utm<-st_read(file.path(preprocessed_greenland_data_path,"shrunk100_scoresby_sound_utm.shp"))
shrunk100_scoresby_sound_utm <- st_transform(shrunk100_scoresby_sound_utm, crs = "+init=EPSG:32626 +units=km")

shrunk200_scoresby_sound_utm<-st_read(file.path(preprocessed_greenland_data_path,"shrunk200_scoresby_sound_utm.shp"))
shrunk200_scoresby_sound_utm <- st_transform(shrunk200_scoresby_sound_utm, crs = "+init=EPSG:32626 +units=km")

shrunk300_scoresby_sound_utm<-st_read(file.path(preprocessed_greenland_data_path,"shrunk300_scoresby_sound_utm.shp"))
shrunk300_scoresby_sound_utm <- st_transform(shrunk300_scoresby_sound_utm, crs = "+init=EPSG:32626 +units=km")

```

```{r compute updated translated distance to shore}
res_shrunk100_land=nearest_points(positions=narwhal_positions,border=shrunk100_scoresby_sound_utm)
res_shrunk200_land=nearest_points(positions=narwhal_positions,border=shrunk200_scoresby_sound_utm)
res_shrunk300_land=nearest_points(positions=narwhal_positions,border=shrunk300_scoresby_sound_utm)

cat("Percentage of points on land with distance from 100m shrunk land polygons : \n",
    length(which(res_shrunk100_land$min_distances==0))/length(dataprep$time)*100,"\n")

cat("Percentage of points on land with distance from 200m shrunk land polygons : \n",
    length(which(res_shrunk200_land$min_distances==0))/length(dataprep$time)*100,"\n")

cat("Percentage of points on land with distance from 300m shrunk land polygons : \n",
    length(which(res_shrunk300_land$min_distances==0))/length(dataprep$time)*100,"\n")

```

```{r distribution of points close to the shore for shrunk polygons}

df100=data.frame(DistanceShore=res_shrunk100_land$min_distances[res_shrunk100_land$min_distances<1.5])
df200=data.frame(DistanceShore=res_shrunk200_land$min_distances[res_shrunk200_land$min_distances<1.5])
df300=data.frame(DistanceShore=res_shrunk300_land$min_distances[res_shrunk300_land$min_distances<1.5])

df100$Category="100m"
df200$Category="200m"
df300$Category="300m"

df<- rbind(df100,df200,df300)
histo=ggplot(df, aes(x =DistanceShore,fill=Category)) +
  geom_histogram(bins=100,color="black")+xlab("Computed distance to shore (km)")
...
add_boundary_data<-function(
    data,res,names=list(px="px",py="py",DistanceShore="DistanceShore",nx="nx",ny="ny")) 
  
  {
  
  #data : dataframe to which wa add columns 
  # res : list as returned by nearest points, with elements
  # nearest points and min_distances
  # names for the distance, nearest points coordinates and normal vectors
  

  nearest_points=res$nearest_points
  min_distances=res$min_distances
  
  #add columns px and py with coordinates of the nearest point on the shore line
  data[[names$px]]=nearest_points[,1]
  data[[names$py]]=nearest_points[,2]
  
  # add columns with difference between narwhal position and nearest point on the shoreline
  data[[names$nx]]=data$x- data[[names$px]]
  data[[names$ny]]=data$y-data[[names$py]]
  
  #replace distance to shore with distances calculated from land polygons (in km)
  data[[names$DistanceShore]]=min_distances
  
  return(data)
}

dataprep<- add_boundary_data(dataprep,res_shrunk100_land,
                  names=list(px="px100",py="py100",DistanceShore="DistanceShore100",nx="nx100",ny="ny100"))

dataprep <- add_boundary_data(dataprep,res_shrunk200_land,
                  names=list(px="px200",py="py200",DistanceShore="DistanceShore200",nx="nx200",ny="ny200"))

dataprep <- add_boundary_data(dataprep,res_shrunk300_land,
                  names=list(px="px300",py="py300",DistanceShore="DistanceShore300",nx="nx300",ny="ny300"))
...
dataprep$Ishore100 <- ExposureFunction(X = dataprep$DistanceShore100) 
dataprep$Ishore200 <- ExposureFunction(X = dataprep$DistanceShore200) 
dataprep$Ishore300 <- ExposureFunction(X = dataprep$DistanceShore300) 
...
dataprep$theta100=rep(NA,n)
dataprep$theta200=rep(NA,n)
dataprep$theta300=rep(NA,n)
...
for (id in unique(dataprep$ID)) {
  ...
  normal100=as.matrix(sub_data[2:n_sub,c("nx100","ny100")])
  normal200=as.matrix(sub_data[2:n_sub,c("nx200","ny200")])
  normal300=as.matrix(sub_data[2:n_sub,c("nx300","ny300")])
  
  #angle between velocity and normal vector
  theta100=signed_angle(vexp_df,normal100)
  theta200=signed_angle(vexp_df,normal200)
  theta300=signed_angle(vexp_df,normal300)
  ...
  theta100=c(NA,theta100)
  theta200=c(NA,theta200)
  theta300=c(NA,theta300)
  vel_phase=c(NA,vel_phase,NA)
  
  dataprep[sub_ind,"AngleVelocities"]=angle_vel
  dataprep[sub_ind,"theta100"]=-theta100
  dataprep[sub_ind,"theta200"]=-theta200
  dataprep[sub_ind,"theta300"]=-theta300
...
plot_AngleNormal=ggplot()+geom_line(data=dataprep[dataprep$DistanceShore>0.05,],aes(time,theta100,col=ID))+
  geom_rect(data = regions, aes(xmin = start_time, xmax = end_time, ymin = -Inf, ymax = Inf),alpha = 0.3,col="lightblue")+
  facet_grid(ID ~.,labeller=as_labeller(c("Asgeir"="A1","Frederik"="A2","Helge18"="A3","Kyrri"="A4","Nemo"="A5","Siggi"="A6")))+
...
AngleNormal_histo=ggplot(dataprep[dataprep$DistanceShore>0.05,], aes(x = theta100,col=ID)) +
  geom_histogram(fill="white",bins=50)+facet_grid(ID ~.,labeller=as_labeller(c("Asgeir"="A1","Frederik"="A2","Helge18"="A3","Kyrri"="A4","Nemo"="A5","Siggi"="A6")))+
  scale_color_viridis_d(guide="none")+
...
Dshore100_histo=ggplot(dataprep, aes(x = DistanceShore100)) +
  geom_histogram(fill="white",color="black",bins=50)+
  facet_grid(ID ~.,labeller=as_labeller(c("Asgeir"="A1","Frederik"="A2","Helge18"="A3","Kyrri"="A4","Nemo"="A5","Siggi"="A6")))+
  theme(legend.position="none")+xlab("Distance to shore (km)")

Dshore200_histo=ggplot(dataprep, aes(x = DistanceShore200)) +
  geom_histogram(fill="white",color="black",bins=50)+
  facet_grid(ID ~.,labeller=as_labeller(c("Asgeir"="A1","Frederik"="A2","Helge18"="A3","Kyrri"="A4","Nemo"="A5","Siggi"="A6")))+
  theme(legend.position="none")+xlab("Distance to shore (km)")

Dshore300_histo=ggplot(dataprep, aes(x = DistanceShore300)) +
  geom_histogram(fill="white",color="black",bins=50)+
  facet_grid(ID ~.,labeller=as_labeller(c("Asgeir"="A1","Frederik"="A2","Helge18"="A3","Kyrri"="A4","Nemo"="A5","Siggi"="A6")))+
  theme(legend.position="none")+xlab("Distance to shore (km)")


ggsave(path=plots_folder,"Dshore100_histo.png", plot = Dshore100_histo)
ggsave(path=plots_folder,"Dshore200_histo.png", plot = Dshore200_histo)
ggsave(path=plots_folder,"Dshore300_histo.png", plot = Dshore300_histo)
```
...
D_up=3
df=cropped_data
df[df$DistanceShore<D_low,"Ishore100"]=1/D_low
df[df$DistanceShore>D_up,"Ishore100"]=0

pIshore=ggplot(df,aes(time,Ishore100,col=ID))+ geom_line(color="black")+facet_grid(ID ~.,labeller=as_labeller(c("Asgeir"="A1","Frederik"="A2","Helge18"="A3","Kyrri"="A4","Nemo"="A5","Siggi"="A6")))+
  theme(legend.position="none")+xlab("Time (h)")+ylab("Inverse distance to shore")



ggsave(path=plots_folder,"realIshore100_through_time.png", plot=pIshore)
```
...
plot_trajectories_land <- ggplot() +
  geom_sf(data=shrunk100_scoresby_sound_utm$geometry,fill="grey")+
  coord_sf(datum=st_crs("+init=EPSG:32626 +units=km"))+
  geom_path(data=dataprep, 
            aes(x = x, y = y, color =ExpShip,group=ID),size = 1, 
            lineend = "round",alpha=0.15) +     
  geom_point(data=dataprep,
...
anim <- plot_trajectories_land + 
  transition_reveal(along = Posixct) + 
  labs(subtitle = "Time: {round(frame_along,1)} hours") 

# Save the animation
animate(anim, nframes = 100, fps = 5, width = 800, height = 600, 
        renderer = gifski_renderer("trajectories_ExpShip.gif"))
