...
\item Converting column "Type" in factor with levels trial and intertrial, and adding two columns "ExpShipT" and "ExpShipI" for exposure to shio during trial and intertrial separately
...
We read the preprocessed land polygons extracted from OpenStreetMap database.
...
For the rest of the analysis, we'll keep the bounded domain defined by the updated polygons in "updated_scoresby_sound_utm" sf object as a reference.
...
for (id in unique(data$ID)) {
  ...
  dataBE1 =rbind(dataBE1,subset(narwhalData, !(time > (maxtime-1)) & (time>tagtime)))
}
...
for (id in unique(data$ID)) {
  narwhalData=dataprep[dataprep$ID==id,]
  maxtime <- min(narwhalData$time[(narwhalData$LOS == 1)])
  dataBE2 =rbind(dataBE2,subset(narwhalData, !(time > (maxtime-1)) & (time>tagtime/2)))
}
...
write.csv(dataBE1,file.path(preprocessed_narwhal_data_path,"DataBE1.csv"))
write.csv(dataBE2,file.path(preprocessed_narwhal_data_path,"DataBE2.csv"))
