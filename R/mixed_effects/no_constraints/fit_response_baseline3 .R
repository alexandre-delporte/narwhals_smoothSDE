...
response8offset <- SDE$new(formulas = formulas,data = dataAE,type = "CTCRW",response = c("x","y"),
                           par0 = par0,other_data=list("log_sigma_obs0"=log(sigma_obs)),map=map,fixpar=c("mu1","mu2"))
response8offset$fit()
...
response9offset <- SDE$new(formulas = formulas,data = dataAE,type = "CTCRW",response = c("x","y"),
                           par0 = par0,map=map,other_data=list("log_sigma_obs0"=log(sigma_obs)),fixpar=c("mu1","mu2"))
response9offset$fit()
