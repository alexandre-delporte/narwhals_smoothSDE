...
formulas <- list(tau =~s(ID,bs="re"),nu=~s(ID,bs="re"),
                   omega=~s(theta,k=6,bs="cs"),D0=~1)

# Fit baseline
baseline_crcvm_cubic<- SDE$new(formulas = formulas,data = renamed_dataBE12,type = "CRCVM_SSM",
                    response = c("x","y"),par0 = c(2,4,0,0.05),other_data=list("H"=H),
                    fixpar=c("D0"))

baseline_crcvm_cubic$update_lambda(c(exp(3),exp(3),10^-12))
baseline_crcvm_cubic$update_map(list("log_lambda"=factor(c(1,2,NA))))
