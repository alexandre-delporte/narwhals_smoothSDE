...
fit_baseline<-function(data,interpolation_data,sigma_obs=0.05,a=2,b=1,D0=0.3,D1=0.5,
  ...
  baseline <- SDE$new(formulas = formulas,
                              ...
                              response = response,par0 = c(2,1,a,b,D0,D1,sigma_D,sigma_theta),
                              fixpar=fixpar,
                              other_data=
...
make_final_baseline_tab<-function(sde,include_sigma_obs=FALSE,n_post=100) {
  
  est=get_baseline_estimates(sde,include_sigma_obs,n_post)
...
#' Make plot to compare BoundaryDistance in simulation and observation
#' @param simulated_data list of data frames simulated from an SDE model, each containing a column "BoundaryDistance".
#' @param observed_data real GPS data (data frame).
#' @return ggplot of boundary distances distributions
make_distance_density_plot <- function(simulated_data, observed_data) {
  ...
  plot_combined <- ggplot(combined_data, aes(x = BoundaryDistance, color = Source, fill = Source)) +
    geom_density(data = simulated_combined, aes(group = SimulationID), alpha = 0.1) + 
    geom_density(data = observed_combined, alpha = 0.2) +  # Single density for observed data
...
#' Make plot to compare BoundaryAngle in simulation and observation
#' @param simulated_data data simulated from a sde model containing a column "BoundaryAngle".
#' @param observed real GPS data
#' @return ggplot of boundary angle distributions
make_angle_density_plot <- function(simulated_data, observed_data) {
  ...
  simulated_combined$BoundaryAngle<-abs(simulated_combined$BoundaryAngle)
  observed_combined$BoundaryAngle<-abs(observed_combined$BoundaryAngle)
  combined_data <- bind_rows(simulated_combined, observed_combined)
  
  

  # Plot density distribution
  plot_combined <- ggplot(combined_data, aes(x = BoundaryAngle, color = Source, fill = Source)) +
    geom_density(data = simulated_combined, aes(group = SimulationID), alpha = 0.1) + 
    geom_density(data = observed_combined, alpha = 0.2) +  # Single density for observed data
...
#' Make plot to compare speed in simulation and observation
#' @param simulated_data data simulated from a sde model containing a column "speed".
#' @param observed real GPS data
#' @return ggplot of boundary angle distributions
make_speed_density_plot <- function(simulated_data, observed_data,response) {
  ...
  # Plot density distribution
  plot_combined <- ggplot(combined_data, aes(x = speed, color = Source, fill = Source)) +
    geom_density(data = simulated_combined, aes(group = SimulationID), alpha = 0.1) + 
    geom_density(data = observed_combined, alpha = 0.2) +  # Single density for observed data
    xlab("Speed") +
    scale_color_manual(values = c("Simulated" = "blue", "Observed" = "red")) + 
...
```{r}
D0=quantile(dataBE12_prep200$BoundaryDistance,0.8)
D1=quantile(dataBE12_prep200$BoundaryDistance,0.5)
a=0.3*D0

baseline_crcvm200<- fit_baseline(dataBE12_prep200,interpolation_bas_200,
                    b=0.5,a=a,sigma_D=1,D0=D0,D1=D1,sigma_theta=pi/6,
                    sigma_obs=0.05)
...
```{r}
make_final_baseline_tab(baseline_crcvm200,include_sigma_obs=FALSE,n_post=10000)
```
...
all_sim_sub<-lapply(all_sim,function(data) {
  
  data_sub<-data[seq(1,nrow(data),by=60),]
  noise<-rmvn(nrow(data_sub),rep(0,2),diag(rep(0.05^2,2)))

  data_sub[,c("x","y")]=data_sub[,c("x","y")]+noise
  return (data_sub)
})
...
all_sim_sub[[1]]$type <- "Simulated"

combined_data <- rbind(dataBE12_prep200[,c("BoundaryDistance","BoundaryAngle","ID","time","x","y","type")], all_sim_sub[[1]])
...
plot_distance<-make_distance_density_plot(all_sim_sub,dataBE12_prep200) 
plot_angle<-make_angle_density_plot(all_sim_sub,dataBE12_prep200)
plot_speed<-make_speed_density_plot(all_sim_sub,dataBE12_prep200)

final_densities_plots<-ggarrange(plotlist=list(plot_distance,plot_angle),
                     ncol=2, nrow=1, common.legend = TRUE, legend="bottom", vjust=2)
ggsave("BoundaryDistance_density.pdf",path="baseline_crcvm_200",plot=plot_distance)
ggsave("BoundaryAngle_density.pdf",path="baseline_crcvm_200",plot=plot_angle)
ggsave("speed_density.pdf",path="baseline_crcvm_200",plot=plot_speed)
```
...
colnames(dataAE_prep200)[colnames(dataAE_prep100)=="DistanceShore200"]<-"BoundaryDistance"
colnames(dataAE_prep200)[colnames(dataAE_prep100)=="theta200"]<-"BoundaryAngle"
