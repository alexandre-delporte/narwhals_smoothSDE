...
dataBE24=read.csv(file.path(narwhal_data_path,"DataBE24.csv"), header = TRUE,dec = ".")
...
The first model is a CTCRW where the covariate theta influences the persistence of the movement.
...
```{r estimates baseline_ctcrw0}
raw_tab_baseline_ctcrw0=get_raw_latex_tab(baseline_ctcrw0)
```
...
get_final_baseline_tab<-function(sde,include_sigma_obs=FALSE,show=TRUE) {
...
final_tab_baseline_ctcrw0=get_final_baseline_tab(baseline_ctcrw0,include_sigma_obs=FALSE)
...
baseline_ctcrw0$plot_par(var="theta_translated",
...
We start by introducing the covariate \texttt{theta} in the new smooth parameter $\omega$ to understand how the animals may rotate depending on the direction of their movement with respect to the shoreline.
...
# Fit baseline with measurement error estimated from the data
baseline_racvm0<- SDE$new(formulas = formulas,data = dataBE12,type = "RACVM_SSM",response = c("x","y"),
...
```{r estimates baseline_racvm0}
raw_tab_baseline_racvm0=get_raw_latex_tab(baseline_racvm0)
```
...
final_tab_baseline_racvm0=get_final_baseline_tab(baseline_racvm0,include_sigma_obs=FALSE)
...
## Baseline with AngleNormal and DistanceShore in omega and tau

We add the effect of the distance to the shore through the covariate \texttt{Ishore_translated}.
...
The smoothing penalties estimates are extremely low (the smoothing penalties are the inverse of the square root of \texttt{omega.te(theta\_translated,Ishore\_translated)}). This suggests that the optimization did not go well and numerical errors occurred. For this reason, we decide to fix the smoothing penalties to "reasonable" values in the tensor splines.

First, we get good values for the smoothing penalty by approximating an expected "reasonable" surface with tensor splines, using the parametric function defined in the paper.

```{r}

fomega=function(cov_data,a=1,b=2,D0=0.3,D1=1,sigma_D=0.3,sigma_theta=pi/6){
...
```{r estimates baseline_racvm1}
raw_tab_baseline_racvm1=get_raw_latex_tab(baseline_racvm1)
```

```{r final estimates baseline_racvm1, message=FALSE}

final_tab_baseline_racvm1=get_final_baseline_tab(baseline_racvm1,include_sigma_obs=FALSE)
...
plots=lapply(1:length(Ishore_values),function(i){
  baseline_racvm1$plot_par(var="theta_translated",covs=data.frame("ID"="Asgeir","Ishore_translated"=Ishore_values[i]),
...
```
Now, we would like to demonstrate how the Distance to shore influences the smooth parameter $\omega$.
...
ggsave("omega_DistanceShore_levels.pdf", plot=shore_influence_plot_baseline_racvm1, path="baseline_racvm1", width=10, height=6, units="in")
...
estimate_parametric_omega=function(model,fixed_a=1) {
    ...
    Dshore=seq(from=quantile(data$DistanceShore_translated,0.5),
               to=quantile(data$DistanceShore_translated,0.95),length.out=30)
    Ishore=1/Dshore
    ...
    z_pred_starting <- fomega(grid,fixed_a,exp(est_par[1]),
                              exp(est_par[2]),exp(est_par[3]),exp(est_par[4]),exp(est_par[5]))
    
    # Reshape z_pred_starting to a matrix for plotting
    z_pred_matrix <- matrix(z_pred_starting, nrow = length(Dshore), ncol = length(theta_translated))
    
    
    # Create the 3D surface plot using plot_ly
    plot_parametric <- plot_ly(x = ~theta_translated, y = ~Dshore, z = ~z_pred_matrix) %>%
      add_surface() %>%
...
res_racvm1=estimate_parametric_omega(baseline_racvm1,fixed_a=2)
...
We can try to fit the exact same model on all the data to check if we get the same kind of results with tighter confidence intervals. Here, we fix the measurement error standard deviation because we get a non positive definite Hessian matrix in the optimization otherwise.
...
final_tab_all_racvm1=get_final_baseline_tab(all_racvm1,include_sigma_obs=FALSE)

```


We now come back to the data before exposure and try a more parsimonious model by removing the dependence of $\tau$ on
...
formulas <- list(mu1 = ~1 ,mu2 =~1,tau =~s(ID,bs="re"),nu=~s(ID,bs="re"),
                   omega=~te(theta_translated,Ishore_translated,k=c(5,5),bs=c("cs")))
...
final_tab_baseline_racvm2=get_final_baseline_tab(baseline_racvm2,include_sigma_obs=FALSE)
...
plots=lapply(1:length(Ishore_values),function(i){
  baseline_racvm2$plot_par(var="theta_translated",covs=data.frame("ID"="Asgeir","Ishore_translated"=Ishore_values[i]),
                         par_names=c("omega"),n_post=100)})
plots
...
xmax=list("Ishore_translated"=1/0.3)
link=list("Ishore_translated"=\(x) 1/x)
xlabel=list("Ishore_translated"="Distance to shore")
...
res_racvm2=estimate_parametric_omega(baseline_racvm2,fixed_a=2)

res_racvm2
```


## Simulate trajectories before exposure from fitted models
...
data_reg=data.frame("AngleNormal"=rep(0,n),"Ishore"=rep(1,n),"Ishore_translated"=rep(1,n),
                    "theta"=rep(0,n),"DistanceShore"=rep(1,n),"theta_translated"=rep(0,n))
...
```{r}
est_par_racvm1=res_racvm1$est_par

formulas=list(tau=~s(theta_translated,k=10,bs="cc")+s(ID,bs="re"),nu=~s(ID,bs="re"),a=~1,b=~1,D0=~1,D1=~1,
              sigma_D=~1,sigma_theta=~1)
baseline_crcvm1<-SDE$new(formulas=formulas,data=dataBE12,type="CRCVM_SSM",response=c("x","y"),
               par0=c(baseline_racvm1$par()[1,c("tau","nu")],2,
                        exp(est_par_racvm1)))

baseline_crcvm1$update_coeff_re(baseline_racvm1$coeff_re()[1:20,])
baseline_crcvm1$update_lambda(baseline_racvm1$lambda()[1:3,])

baseline_crcvm1_trajectory=baseline_crcvm1$simulate(z0=z0_BE,data=data_reg,atw=atw,land=land,verbose=FALSE)


n=length(baseline_crcvm1_trajectory$time)

baseline_crcvm1_trajectory_sub=baseline_crcvm1_trajectory[seq(1,n,by=60),]

plot_baseline_crcvm1_trajectory=ggplot() +
  geom_sf(data=land$geometry,fill="grey")+
  coord_sf(datum=st_crs("+init=EPSG:32626 +units=km"))+
  geom_path(data=baseline_crcvm1_trajectory_sub, 
            aes(x = x, y = y,color=ID),size = 0.5, 
            lineend = "round",alpha=0.15) +     
  geom_point(data=baseline_crcvm1_trajectory_sub,
             aes(x = x, y = y,color=ID),size =0.5 ) +
  theme_minimal()
```
...
formulas=list(tau=~s(ID,bs="re"),nu=~s(ID,bs="re"),a=~1,b=~1,D0=~1,D1=~1,
              sigma_D=~1,sigma_theta=~1)
baseline_crcvm2<-SDE$new(formulas=formulas,data=dataBE12,type="CRCVM_SSM",response=c("x","y"),
               par0=c(baseline_racvm2$par()[1,c("tau","nu")],2,
                        exp(est_par_racvm2)))

baseline_crcvm2$update_coeff_re(baseline_racvm2$coeff_re()[1:12,])
baseline_crcvm2$update_lambda(baseline_racvm2$lambda()[1:2,])

baseline_crcvm2_trajectory=baseline_crcvm2$simulate(z0=z0_BE,data=data_reg,atw=atw,land=shrunk_land,verbose=FALSE)
...
ggsave("plot_baseline_crcvm2_trajectory.pdf", plot=plot_baseline_crcvm2_trajectory,
       path="baseline_racvm2", width=10, height=6, units="in")
...
baseline_crcvm1_trajectory_sub <- baseline_crcvm1_trajectory_sub %>% 
  mutate(data_type = "Simulated baseline_crcvm1")
...
baseline_trajectories <- bind_rows(baseline_crcvm1_trajectory_sub,
                                   baseline_crcvm2_trajectory_sub,observed_trajectory)
...
# add noise
noise=rmvn(length(baseline_crcvm2_trajectory_sub$time),rep(0,2),diag(rep(0.05^2,2)))
baseline_crcvm2_trajectory_sub_obs=baseline_crcvm2_trajectory[seq(1,n,by=30),c("time","x","y","ID")]
baseline_crcvm2_trajectory_sub_obs[,c("x","y")]=baseline_crcvm2_trajectory_sub_obs[,c("x","y")]+noise
baseline_crcvm2_trajectory_sub_obs=add_covs_parallel(baseline_crcvm2_trajectory_sub_obs)
...
#Fixed measurement error
sigma_obs=0.05
H=array(rep(sigma_obs^2*diag(2),length(baseline_crcvm2_trajectory_sub_obs$time)),
        dim=c(2,2,length(baseline_crcvm2_trajectory_sub_obs$time)))
...
# Fit baseline
baseline_racvm2_check<- SDE$new(formulas = formulas,data = baseline_crcvm2_trajectory_sub_obs,type = "RACVM_SSM",
                    response = c("x","y"),par0 = par0,other_data=list("H"=H),
                    fixpar=c("mu1","mu2"))
...
```
```{r}
get_final_baseline_tab(baseline_racvm2_check,include_sigma_obs=FALSE)
```
...
final_tab_response_exp=get_final_response_tab(response_racvm_exp)
...
```{r}
get_recovery_distances_tab<-function(p) {
...
get_recovery_distances_tab(0.5)

get_recovery_distances_tab(0.1)
```
