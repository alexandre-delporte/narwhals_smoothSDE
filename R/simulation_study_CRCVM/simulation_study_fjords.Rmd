...
ftau_bump=function(cov_data,epsilon=pi/8,alpha=pi/5,tau0=0.2,tau1=2) {
    Dshore=cov_data$DistanceShore
    theta=cov_data$theta
...
fomega_fast=function(cov_data,D0=0.05,omega0=120*pi,lambda=2,kappa=0.2) {
    Dshore=cov_data$DistanceShore
    theta=cov_data$theta
    if (is.null(Dshore)){
      Dshore=1/cov_data$ExpShore
...
Dshore=seq(from=0.01,to=2,length.out=30)
...
true_omega=matrix(fomega_fast(grid),30,30)
...
data_fjords_hf_ne <- foreach (i=1:N_ID,.combine='rbind',.packages=c("progress","MASS","sf")) %dopar% {
  
    res=sim_theta_CRCVM(ftau_constant,fomega_fast,fnu_constant,log_sigma_obs=log(sigma_obs),v0,x0[i,],times_hf,land=land,verbose=FALSE)
...
plot_samples_fjords_hf_ne=ggplot()+geom_sf(data=land$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_fjords_hf_ne,mapping=aes(y1,y2,color=ID),size=0.1)+
  geom_path(data=data_fjords_hf_ne,mapping=aes(y1,y2,color=ID),size=0.1)+
  geom_point(data = data_fjords_hf_ne%>% filter(!duplicated(ID)),
...
```
```{r plot subsampled data}

data_fjords_lf_ne=data_fjords_hf_ne[seq(1,length(data_fjords_hf_ne$time),by=50),]
plot_samples_fjords_lf_ne=ggplot()+geom_sf(data=land$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_fjords_lf_ne,mapping=aes(y1,y2,color=ID),size=0.1)+
  geom_path(data=data_fjords_lf_ne,mapping=aes(y1,y2,color=ID),size=0.1)+
  geom_point(data = data_fjords_lf_ne%>% filter(!duplicated(ID)),
             aes(x = y1, y = y2), shape = 3, size = 4, col = "red")+
  xlab("x") + ylab("y")


ggsave(filename="samples_fjords_lf_ne_omega.png",plot=plot_samples_fjords_lf_ne,width=10,height=5)
...
crcvm_fjords_hf_ne1<- foreach(k=1:N_BATCH,.packages=c("smoothSDE")) %do% {
...
p=ggplotly(p)
saveWidget(p, file ="crcvm_fjords_hf_ne1/all_batches_coeffs_crcvm_fjords_hf_ne1.html")
...
##  Estimate with DistanceShore covariate and tensor splines te
...
crcvm_fjords_hf_ne2<- foreach(k=1:N_BATCH,.packages=c("smoothSDE")) %do% {
  #estimation with tensor splines
  formulas <- list(mu1=~1,mu2=~1,tau=~s(ID,bs="re"),
...
```{r get plots from hf e estimates with DistanceShore}
...
```{r violin plots spline coeff te DistanceShore for one batch of low error samples}
...
n_coeffs=length(sp_coeff_DistanceShore)
coeffs_df=data.frame("estimate"=NA,"coeff_name"=NA)
