...
for (domain in domains) {
  ...
  for (hyperparams_file in hyperparams_files_list) {
    ...
    out_files<- list.files(path=file.path(domain,paste0("results_",hyperparams_file_name)),
                           pattern ="\\.out$")
    ...
    for (file in out_files) {
      ...
      file_content <- readLines(file.path(domain,paste0("results_",hyperparams_file_name),file))
    ...
    surface_files<- list.files(path=file.path(domain,paste0("results_",hyperparams_file_name)),pattern =pattern)
