...
for (i in 1:N_ID) {
  ...
  x=c(runif(1,min=2,max=8),runif(1,min=2,max=8))
  x0[i,]=x
}
...
fomega=function(cov_data,D0=0.3,omega0=60*pi/2,lambda=2,kappa=0.3) {
...
#fit with bivariate splines te for DistanceShore
knots_DistanceShore=list(theta=seq(-pi,pi,len=SP_DF),DistanceShore=seq(0.2,4,len=SP_DF))
...
knots_ExpShore=list(theta=seq(-pi,pi,len=SP_DF),ExpShore=seq(1/4,1/0.2,len=SP_DF))
