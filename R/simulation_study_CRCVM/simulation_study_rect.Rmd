...
source(file.path(par_dir,"CVM_functions.R"))
```
...
Dshore=seq(from=0.3,to=4,length.out=30)
theta=seq(from=-pi,to=pi,length.out=30)
grid<- as.data.frame(expand.grid(Dshore,theta))
colnames(grid)=c("DistanceShore","theta")
...
```{r generate batch of hf low errors samples,cache=TRUE}


data_rect_hf_ne =foreach (i=1:N_ID,.combine='rbind') %do% {
...
plot_samples_rect_hf_ne=ggplot()+geom_sf(data=domain$geometry,fill="grey")+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_rect_hf_ne,aes(p1,p2),shape=5,size=0.1,col="red")+
  geom_point(data=data_rect_hf_ne,mapping=aes(y1,y2,color=ID,shape=ID),size=0.1)+
  geom_path(data=data_rect_hf_ne,mapping=aes(y1,y2,color=ID,shape=ID),size=0.1)+
  geom_point(data = data_rect_hf_ne%>% filter(!duplicated(ID)),
