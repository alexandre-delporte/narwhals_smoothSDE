...
This type of behavior can be obtained with several function.
We start by using the hyperbolic tangent function.

```{r parametric tanh function for omega}

fomega_tanh=function(cov_data,D0=0.5,omega0=30*pi/2,lambda=2,kappa=0.2) {
  Dshore=cov_data$BoundaryDistance
  theta=cov_data$BoundaryAngle
...
get_omega_plot<-function(fomega,Dshore,theta) {
  
  #grid of values
  grid<- as.data.frame(expand.grid(Dshore,theta))
  colnames(grid)=c("BoundaryDistance","BoundaryAngle")

  #smooth omega
  npoints=length(Dshore)
  omega=matrix(fomega(grid),npoints,npoints)
...
get_omega_plot(fomega_tanh,Dshore,theta)
...
get_omega_plot(fomega_sqrt,Dshore,theta)

```


These smooth functions $\omega$ will produce a SDE model that is constrained within a restricted domain of $\R^2$.
The trajectories avoid the shoreline, but are not attracted to it. To model a tendency to follow the shoreline, we need to modify 
a bit the smooth function to get a rotation of the velocity toward the shore when it is on the verge of leaving it. 
We can do this in several ways. First, we use a sin function.



```{r define and plot omega sin}
# Define the extended function f(theta)
fomega_sin<- function(cov_data, a =4*pi,b=10*pi,D0=0.8) {
  Dshore=cov_data$BoundaryDistance
  theta=cov_data$BoundaryAngle
...
get_omega_plot(fomega_sin,Dshore,theta)

```

In these functions, there is no mid range effect of the shore on the drift of the process, which means
that when the process is far enough from the shore, it cannot be attracted back.
To resolve this, we add a gaussian kernel in the functions.

```{r}

fomega_gaussian= function(cov_data,b=1,D1=1.5,theta1=pi/2/sqrt(3),sigma_D=1,sigma_theta=0.6) {
  Dshore=cov_data$BoundaryDistance
  theta=cov_data$BoundaryAngle
  b*(exp(-1/2*(theta+theta1)^2/sigma_theta^2)-
           exp(-1/2*(theta-theta1)^2/(sigma_theta)^2))*exp(-1/2*(Dshore-D1)^2/sigma_D^2)
}

get_omega_plot(fomega_gaussian,Dshore,theta)

```

We define a product of sine and cosine with an additional gaussian attractive term

```{r define and plot omega sin}
# Define the extended function f(theta)
fomega_sin_cos<- function(cov_data,omega_0=10,D0=0.8,p=1,b=1,D1=1,theta1=pi/4,sigma_D=1,sigma_theta=0.5) {
  Dshore=cov_data$BoundaryDistance
  theta=cov_data$BoundaryAngle
  
  attraction <- fomega_gaussian(cov_data,b,D1,theta1,sigma_D,sigma_theta)
  
  if (p%%2==0) {
    repulsion <- -omega_0*sign(theta)*sin(theta/2)^p*cos(theta)*D0/Dshore*exp(-Dshore/D0)
  }
  else {
    repulsion <- -omega_0*sin(theta/2)^p*cos(theta)*D0/Dshore*exp(-Dshore/D0)
  }
  attraction+repulsion
}

get_omega_plot(fomega_sin_cos,Dshore,theta)

```



Now we use a polynomial with an additional gaussian kernel to model attraction toward the shore.

```{r}
fomega_poly=function(cov_data,a=0.2,b=5,D0=0.3,D1=1.5,sigma_D=1,sigma_theta=pi/8,
                     theta1=pi/2/sqrt(3),p=3) {
  Dshore=cov_data$BoundaryDistance
  theta=cov_data$BoundaryAngle
  
  if (p%%2==0) {
    repulsion<- a*sign(theta)*theta^p*(theta-pi/2)*(theta+pi/2)*D0/Dshore*exp(-Dshore/D0)
  }
  else {
    repulsion<- a*theta^p*(theta-pi/2)*(theta+pi/2)*D0/Dshore*exp(-Dshore/D0)
  }
...
get_omega_plot(fomega_poly,Dshore,theta)
```
```{r}
fomega_quadratic=function(cov_data,a=0.2,D0=0.3,D1=1.5) {
  Dshore=cov_data$BoundaryDistance
  theta=cov_data$BoundaryAngle
  
    return (a*(theta-pi/2)*(theta+pi/2)*D0/Dshore*exp(-Dshore/D0))
}

get_omega_plot(fomega_quadratic,Dshore,theta)
```
...
get_omega_plot(fomega_minimal,Dshore,theta)
```



We create and save the perpective plot for the paper.

```{r persp plot cubic parametric omega}

#Define function omega with adequate parameters
fomega_cubic<-function(cov_data) {
  fomega_poly(cov_data,a=2,b=5,D0=0.3,D1=0.8,sigma_D=0.3,sigma_theta=pi/8,
                     theta1=pi/2/sqrt(3),p=1)
}

# Perspective plots for omega

# Grid 
Dshore = seq(from = 0.05, to = 2, length.out = 30)
theta = seq(from = -pi + 0.1, to = pi - 0.1, length.out = 30)
grid <- as.data.frame(expand.grid(Dshore, theta))
colnames(grid) = c("BoundaryDistance", "BoundaryAngle")

# Smooth omega
z = matrix(fomega_cubic(grid), 30, 30)
...
simulate_paralell<-function(ftau,fnu,fomega,v0=v0,x0=x0,times,border,n_cores=6,seed=42) {
  ...
  #paralell loop
  data=foreach (i=1:N,.combine='rbind',.export=c("sim_CRCVM","fomega_tanh","fomega_poly",
                "fomega_sin","fomega_sin_cos","fomega_sqrt","fomega_gaussian","fomega_minimal","fomega_quadratic"),
                .packages=c("progress","MASS","sf","mgcv","smoothSDE")) %dopar% {
    
    set.seed((seed-1)*N+i)
    
    res=sim_CRCVM(ftau=ftau,fomega=fomega,fnu=fnu,v0=v0,x0=x0[i,],
                      times=times,land=border,verbose=FALSE)
    
    data_sim=res$sim
    data_sim$ID=factor(rep(i,length(data_sim$y1)))
    data_shore=res$boundary[,c("p1","p2")]
...
data_rect_tanh<-simulate_paralell(ftau_constant,fnu_constant,fomega_tanh,v0,x0,times,rect_border)

```

```{r plot rect_tanh, message=FALSE}

get_trajectory_plot<-function(data,border,by=60) {
  
  #subsample to 10 minutes
  data_sub=data[seq(1,nrow(data),by=by),]
  
  #create plot
  plot<- ggplot() +
    geom_sf(data = border$geometry, fill = "grey", inherit.aes = FALSE) +
    coord_sf(datum = st_crs("+init=EPSG:32626 +units=km")) +
    geom_path(data = data_sub, aes(x = y1, y = y2, col = ID), size = 0.1) +
    geom_point(data = data_sub, aes(x = y1, y = y2, col = ID), size = 0.1) +
...
get_trajectory_plot(data_rect_tanh,rect_border)
```

The process is avoiding land by rotating when it approaches the boundary. 
Now, to have trajectories that tends to remain close to the boundary and move along it we can use the function \texttt{fomega_poly} with 
a non zero attractive gaussian term instead.

```{r simulate with omega cubic attractive in rectangle,cache=TRUE}

fomega_cubic_rect=function(cov_data) {
  
  fomega_poly(cov_data,a=1,b=5,D0=0.5,D1=1.5,sigma_D=0.5,sigma_theta=pi/8,p=1)
  
}

data_rect_cubic<-simulate_paralell(ftau_constant,fnu_constant,fomega_cubic_rect,v0,x0,times,rect_border)

```

```{r}

get_trajectory_plot(data_rect_cubic,rect_border)
```

We remove the attractive term in the function by taking parameter $b=0$.

```{r simulate with omega cubic repulsive in rectangle,cache=TRUE,message=FALSE}

fomega_cubic_repulsive_rect=function(cov_data) {
  
  fomega_poly(cov_data,a=1,b=0,D0=0.5,D1=1.5,sigma_D=0.5,sigma_theta=pi/8,p=1)
  
}
  
data_rect_cubic_repulsive<-simulate_paralell(ftau_constant,fnu_constant,fomega_cubic_repulsive_rect,
                                             v0,x0,times,rect_border)
...
```{r}
get_trajectory_plot(data_rect_cubic_repulsive,rect_border)
```

We can also increase the parameter $p$ in the polynomial omega function to get a steeper function
at the extremities.

```{r simulate with omega cubic attractive in rectangle,cache=TRUE,message=FALSE}

fomega_poly_rect=function(cov_data) {
  
  fomega_poly(cov_data,a=1,b=0,D0=0.5,D1=1.5,sigma_D=0.5,sigma_theta=pi/8,p=2)
  
}

data_rect_poly<-simulate_paralell(ftau_constant,fnu_constant,fomega_poly_rect,v0,x0,times,rect_border)

```

```{r}

get_trajectory_plot(data_rect_poly,rect_border)
```

We now try our \texttt{fome_sin_cos} function defined with a product of sin and cosine.

```{r simulate with omega cubic attractive in rectangle,cache=TRUE,message=FALSE}

fomega_sin_cos_rect=function(cov_data) {
  
  
  fomega_sin_cos(cov_data,omega_0=10,D0=0.8,p=1,b=0,D1=1,theta1=pi/4,sigma_D=1,sigma_theta=0.5)
  
}

data_rect_sin_cos<-simulate_paralell(ftau_constant,fnu_constant,fomega_sin_cos_rect,v0,x0,times,rect_border)

```

```{r}

get_trajectory_plot(data_rect_sin_cos,rect_border)
```



```{r simulate with omega minimal in rectangle,cache=TRUE,message=FALSE}

fomega_minimal_rect=function(cov_data) {
  
  fomega_minimal(cov_data,nu=4,D0=0.5,D1=1.5,a=3,b=10,sigma_D=0.5,sigma_theta=pi/8)
}
  

data_rect_minimal<-simulate_paralell(ftau_constant,fnu_constant,fomega_minimal_rect,v0,x0,times,rect_border)
...
get_trajectory_plot(data_rect_minimal,rect_border)
```


Until now, we kept the parameter $\tau$ constant. We could emphasize the tendency to move along the boundary by using a smooth $\tau$ given by the functions \texttt{ftau_tanh} or \texttt{ftau_gaussian}.


Finally, we compare this to Hanks simulation technique for reflected SDE.
...
data_rect_hanks=foreach (i=1:N,.combine='rbind',.packages=c("progress","MASS","sf","mgcv")) %dopar% {
...
get_trajectory_plot(data_rect_hanks,rect_border)
```
...
fomega_cubic_attractive_circ=function(cov_data) {
  
   fomega_poly(cov_data,a=30,D0=0.5,D1=3,sigma_theta=pi/10,sigma_D=1,b=5,p=1,theta1=pi/2)
}

data_circ_cubic_attractive <-simulate_paralell(ftau_constant,fnu_constant,fomega_cubic_attractive_circ,v0,x0,times,circ_border)
...
get_trajectory_plot(data_circ_cubic_attractive,circ_border)
```
...
#change parameter values 
fomega_cubic_repulsive_circ=function(cov_data) {
  
   fomega_poly(cov_data,a=3,D0=0.5,D1=1.5,sigma_theta=pi/10,sigma_D=0.5,b=0,theta=pi/2,p=1)
}

data_circ_cubic_repulsive <-simulate_paralell(ftau_constant,fnu_constant,fomega_cubic_repulsive_circ,v0,x0,times,circ_border)
...
get_trajectory_plot(data_circ_cubic_repulsive,circ_border)
```
...
data_circ_hanks=foreach (i=1:N,.combine='rbind',.packages=c("progress","MASS","sf","mgcv")) %dopar% {
...
```{r}
get_trajectory_plot(data_circ_hanks,circ_border)
```
...
data_circ_island_cubic_attractive=simulate_paralell(ftau_constant,fnu_constant,fomega_cubic_attractive_circ,v0,x0,times,circ_island_border)
...
get_trajectory_plot(data_circ_island_cubic_attractive,circ_island_border)
```


```{r simulate with omega cubic and tau gaussian in circle with islands,cache=TRUE}
data_circ_island_cubic_repulsive<- simulate_paralell(ftau_constant,fnu_constant,fomega_cubic_repulsive_circ,v0,x0,times,circ_island_border)

```

```{r}
get_trajectory_plot(data_circ_island_cubic_repulsive,circ_island_border)
```


# Trajectories in Scoresby Sound fjords

Now we simulate trajectories in the fjords.


```{r simulate with omega cubic and tau gaussian in circle with islands,cache=TRUE}
data_fjords_tanh<- simulate_paralell(ftau_constant,fnu_constant,fomega_tanh,v0,x0,times,land_border)
```

```{r}
get_trajectory_plot(data_fjords_tanh,land_border)
```

```{r define function to get animated trajectories,eval=FALSE}

animate_trajectory_plot<- function(data,border,by=60,anim_name="sim.gif") {
  ...
  # Basic ggplot setup
  plot <- ggplot() +
    geom_sf(data=border$geometry,fill="grey")+
    coord_sf(datum=st_crs("+init=EPSG:32626 +units=km"))+
    geom_path(data=data_sub, 
  ...
  animate(anim, nframes = 100, fps = 5, width = 800, height = 600, 
          renderer = gifski_renderer(anim_name))
  }
...
```{r}
animate_trajectory_plot(data_fjords_tanh,land_border,by=60,anim_name="sim_fjords_tanh.gif")
```

```{r simulate with omega cubic in fjords,cache=TRUE}

#set parameters values
fomega_fjords_cubic_attractive<- function(cov_data){
 fomega_poly(cov_data,a=3,D0=0.5,D1=1,sigma_theta=pi/8,sigma_D=0.5,b=4,theta1=pi/2,p=1)
}

data_fjords_cubic_attractive<- simulate_paralell(ftau_constant,fnu_constant,fomega_fjords_cubic_attractive,v0,x0,times,land_border)
```

```{r}
get_trajectory_plot(data_fjords_cubic_attractive,land_border)
```



```{r}
animate_trajectory_plot(data_fjords_cubic_attractive,land_border,anim_name="sim_fjords_cubic_attractive.gif")
```




![Simulated Trajectories](sim_fjords_cubic_attractive.gif)


We now simulate trajectories without the attractive gaussian term in $\omega$.

```{r simulate with omega cubic repulsive in fjords,cache=TRUE}

fomega_cubic_fjords_repulsive=function(cov_data) {
  fomega_cubic(cov_data,a=1,D0=0.3,D1=0.8,sigma_theta=pi/8,sigma_D=0.3,b=0)
}

data_fjords_cubic_repulsive<- simulate_paralell(ftau_constant,fnu_constant,fomega_fjords_cubic_repulsive,v0,x0,times,land_border)
```



```{r}
get_trajectory_plot(data_fjords_cubic_repulsive,land_border)
```

```{r}
animate_trajectory_plot(data_fjords_cubic_repulsive,land_border,anim_name="sim_fjords_cubic_repulsive.gif")
```

![Simulated Trajectories](sim_fjords_cubic_repulsive.gif)

```{r simulate with omega quadratic in fjords,cache=TRUE}

fomega_fjords_quadratic=function(cov_data) {
  fomega_quadratic(cov_data,a=0.5,D0=3)
}

data_fjords_quadratic<- simulate_paralell(ftau_constant,fnu_constant,fomega_fjords_quadratic,v0,x0,times,land_border)
```


```{r}
get_trajectory_plot(data_fjords_quadratic,land_border)
```
```{r simulate with omega sin cos in fjords,cache=TRUE}

fomega_fjords_sin_cos=function(cov_data) {
  fomega_sin_cos(cov_data,omega_0=10,D0=0.8,p=1,b=0)
}


data_fjords_sin_cos<- simulate_paralell(ftau_constant,fnu_constant,fomega_fjords_sin_cos,v0,x0,times,land_border)
```


```{r}
get_trajectory_plot(data_fjords_sin_cos,land_border)
```
...
data_fjords_hanks=foreach (i=1:N,.combine='rbind',.packages=c("progress","MASS","sf","mgcv")) %dopar% {
...
```{r}
get_trajectory_plot(data_fjords_hanks,land_border)
```

```{r}
animate_trajectory_plot(data_fjords_hanks,land_border,anim_name="sim_fjords_hanks.gif")
```

```{r simulate with omega sin cos in fjords,cache=TRUE}

#set parameters values
fomega_fjords_sin_cos_attractive<- function(cov_data){
 fomega_sin_cos(cov_data,omega_0=20,D0=0.5,p=2,b=3,D1=1,theta1=pi/2/sqrt(3),sigma_D=1,sigma_theta=0.5)
}

data_fjords_sin_cos_attractive<- simulate_paralell(ftau_constant,fnu_constant,
                                                   fomega_fjords_sin_cos_attractive,v0,x0,times,land_border)
```

```{r}
get_trajectory_plot(data_fjords_sin_cos_attractive,land_border)
```
We try a higher order polynomial.


```{r simulate with omega cubic repulsive in fjords,cache=TRUE}

fomega_fjords_poly_attractive=function(cov_data) {
  fomega_poly(cov_data,a=1,D0=0.3,D1=1,sigma_theta=pi/8,sigma_D=1,b=3,p=3)
}

data_fjords_poly_attractive<- simulate_paralell(ftau_constant,fnu_constant,
                                                fomega_fjords_poly_attractive,v0,x0,times,land_border)
```



```{r, message=FALSE}
plot=get_trajectory_plot(data_fjords_poly_attractive,land_border)
```

```{r}
animate_trajectory_plot(data_fjords_cubic_repulsive,land_border,anim_name="sim_fjords_cubic_repulsive.gif")
```
# Create final plots for the paper
...
x.min= min(c(data_sim_fjords_cubic_attractive_sub$y1,
             data_sim_fjords_hanks_sub$y1))-1
x.max = max(c(data_sim_fjords_cubic_attractive_sub$y1,
              data_sim_fjords_hanks_sub$y1))+1
y.min = min(c(data_sim_fjords_cubic_attractive_sub$y2,
              data_sim_fjords_hanks_sub$y2))-1
y.max = max(c(data_sim_fjords_cubic_attractive_sub$y2,
              data_sim_fjords_hanks_sub$y2))+1
...
#plot only trajectory in fjords

data_combined_fjords=data_combined[data_combined$domain=="Fjords",]

# Create a list of plots
plot_list <- lapply(seq_len(length(unique(data_combined$model))), function(i) {
  model <- unique_combinations$model[i]
  create_plot(domain="Fjords",model)
})

final_plot_fjords=ggarrange(plotlist=plot_list,
                     ncol=1, nrow=2, common.legend = TRUE, legend="bottom",
                     labels= c("shore-attractive CVM","shore-reflected CVM"))

final_plot_fjords

ggsave("illustrative_trajectories_fjords.pdf", plot = final_plot_fjords, width = 14, height = 14)
