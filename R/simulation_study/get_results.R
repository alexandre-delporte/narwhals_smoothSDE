...
for (domain in domains) {
  ...
  # Read csv files ----------
  files_DistanceShore<- list.files(path=domain,pattern = "^result_rect_DistanceShore[0-9]+\\.csv$")
  files_ExpShore<- list.files(path=domain,pattern = "^result_rect_ExpShore[0-9]+\\.csv$")
  ...
  write.csv(results_DistanceShore, paste("result_",domain_name,"_DistanceShore.csv",sep=""), row.names=FALSE)
  write.csv(results_ExpShore, paste("result_",domain_name,"_ExpShore.csv",sep=""), row.names=FALSE)
}