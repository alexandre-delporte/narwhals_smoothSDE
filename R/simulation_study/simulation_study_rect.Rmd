...
ggsave(filename="samples_rect_hf_ne_omega.png",plot=plot_samples_rect_hf_ne,width=10,height=5)
...
crcvm_rect_hf_ne1<- foreach(i=1:N_BATCH,.packages=c("smoothSDE")) %do% {
...
crcvm_rect_hf_ne2<- foreach(i=1:N_BATCH,.packages=c("smoothSDE")) %do% {
  #estimation with tensor splines
  formulas <- list(mu1=~1,mu2=~1,tau=~s(ID,bs="re"),
