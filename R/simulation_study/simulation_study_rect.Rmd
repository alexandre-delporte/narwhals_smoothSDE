...
```{r high error samples, cache=TRUE,results='hide'}
#setup parallel backend to use many processors
cores=detectCores()
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
...
ggsave("all_batches_coeffs_crcvm_rect_hf_ne1.png",plot=p,path="crcvm_rect_hf_ne1")
...
crcvm_rect_hf_ne2=list()
for (k in 1:N_BATCH) {
  m <- SDE$new(formulas = formulas,data = data_rect_hf_ne[[paste("BATCH"),k,sep=""]],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2"),
                      other_data=list("H"=H0_hf))
  m$fit(method="BFGS")
  crcvm_rect_hf_ne2[[paste("BATCH",k,sep="")]]<-m
}
```
...
ggsave("all_batches_coeffs_crcvm_rect_hf_ne2.png",plot=p,width=10,height=5,path="crcvm_rect_hf_ne2")
...
ggsave("all_batches_coeffs_crcvm_rect_hf_e1.png",plot=p,width=10,height=5,path="crcvm_rect_hf_e1")
...
for (k in 1:N_BATCH) {
  m <- SDE$new(formulas = formulas,data = data_rect_hf_e[[paste("BATCH",k,sep="")]],type = "RACVM",
                      ...
                      other_data=list("H"=H0_hf))
  m$fit(method="BFGS")
  crcvm_rect_hf_e2[[paste("BATCH",k,sep="")]]<-m
}
```
...
ggsave("all_batches_coeffs_crcvm_rect_hf_e2.png",plot=p,width=10,height=5,path="crcvm_rect_hf_e2")
```