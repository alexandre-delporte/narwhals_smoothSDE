...
#constant nu
fnu_constant=function(cov_data,nu=1,sigma_re=0.3) {
    b=rnorm(1,mean=0,sd=sigma_re)
  return (nu+b)
}
...
data_rect_hf_ne=list()

for (k in 1:N_BATCH) {
  
  data=data.frame("y1"=NA,"y2"=NA,"time"=NA,"p1"=NA,"p2"=NA,"ID"=NA)
  ...
  for (i in 1:N_ID) {
    ...
    data=rbind(data,cbind(data_sim,data_shore))
  }
  
  #for the first value we cannot compute the angle theta
  data=data[-1,]
  data_rect_hf_ne[[paste("BATCH",k,sep="")]]=data
}
...
for (k in 1:N_BATCH) {
  ...
  #for the first value we cannot compute the angle theta
  data=data[-1,]
  data_rect_hf_e[[paste("BATCH",k,sep="")]]=data
}
```
...
crcvm_rect_hf_ne1 <- SDE$new(formulas = formulas,data = data_rect_hf_ne$BATCH1,type = "RACVM",
