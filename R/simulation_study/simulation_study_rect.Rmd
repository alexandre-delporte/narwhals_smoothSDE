...
true_tau=matrix(ftau_constant(grid),30,30)
...
saveWidget(plot_true_omega, file ="true_smooth_omega_rect.html")
...
### Plot spline omega and spline basis functions
...
plot_basis4 <- plot_ly(type = "surface",contours = list(z = list(show = TRUE, start = min(z4), end = max(z4), size = 0.05,color="black")),
                         x = ~Dshore,y = ~theta,z=~z4, colors=colorRamp(c("blue", "lightblue", "chartreuse3", "yellow", "red")))%>%
                layout(title=paste("Spline basis 4"),scene=list(xaxis = list(title = "Theta",showgrid = T),
                                                                 yaxis = list(title = "Distance to shore",showgrid = F),
                                                            zaxis = list(title = "omega")))

saveWidget(plot_spline_omega, file ="spline_smooth_omega_rect.html")
```
...
saveWidget(plot_basis2, file ="spline_basis2_omega_rect.html")
```

```{r plot basis3}
plot_basis3
saveWidget(plot_basis3, file ="spline_basis3_omega_rect.html")
```

```{r plot basis4}
plot_basis4
saveWidget(plot_basis4, file ="spline_basis4_omega_rect.html")
```
...
```{r illustrative sample for the paper}

res=sim_theta_CRCVM(ftau=ftau_constant,fomega=fomega,fnu=fnu_constant,v0=v0,x0=x0[1,],times=seq(0,2*24,by=1/60),land=domain,verbose=FALSE)
...
```{r parallel generate batches of hf low errors samples,cache=TRUE}
...
data_rect_hf_ne <- foreach(i=1:N_BATCH,.packages=c("progress","MASS","sf","foreach")) %dopar% {
  data=foreach (i=1:N_ID,.combine='rbind') %do% {
    ...
    res=sim_theta_CRCVM(ftau_constant,fomega,fnu_constant,log_sigma_obs=log(sigma_obs),v0,x0[i,],times_hf,land=domain,verbose=FALSE)
...
### plot one sample and one batch

```{r plot one batch low error samples}

plot_samples_rect_hf_ne=ggplot()+geom_sf(data=domain$geometry,fill="grey")+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_rect_hf_ne$BATCH1,aes(p1,p2),shape=5,size=0.1,col="red")+
  geom_point(data=data_rect_hf_ne$BATCH1,mapping=aes(y1,y2,color=ID,shape=ID),size=0.1)+
  geom_path(data=data_rect_hf_ne$BATCH1,mapping=aes(y1,y2,color=ID,shape=ID),size=0.1)+
...
ggsave(filename="samples_rect_hf_ne.png",plot=plot_samples_rect_hf_ne,width=10,height=5)
...
crcvm_rect_hf_ne1<- foreach(k=1:N_BATCH,.packages=c("smoothSDE")) %do% {
...
xlabel=list("ExpShore"="Distance to shore","theta"="Theta")
link=list("ExpShore"=(\(x) 1/x),"theta"=identity)
...
res=crcvm_rect_hf_ne1$BATCH1$get_all_plots(baseline=NULL,model_name="crcvm_rect_hf_ne1",xmin=xmin,xmax=xmax,
                                    xlabel=xlabel,link=link,show_CI="pointwise",true_smooths=true_smooths,save=FALSE)
res$fe_omega_theta_ExpShore
```


#### Violin plot of the estimated coefficients for one batch
...
for (k in 1:N_BATCH) {
  ...
  coeffs=rbind(m$coeff_re(),m$coeff_fe())
...
```{r compute biais and rmse}
biais=c()
rmse=c()
for (string in names(sp_coeff_ExpShore)) {
  name=paste("omega.",string,sep="")
  true_value=sp_coeff_ExpShore[string]
  estimates=coeffs_df[coeffs_df$coeff_name==name,"estimate"]
  biais=c(biais,1/N_BATCH*sum(abs(estimates-true_value))/true_value)
  rmse=c(rmse,sqrt(1/N_BATCH*sum((estimates-true_value)^2)))
}

names(biais)=paste("omega.",names(sp_coeff_ExpShore),sep="")
names(rmse)=paste("omega.",names(sp_coeff_ExpShore),sep="")
```
...
crcvm_rect_hf_ne2<- foreach(k=1:N_BATCH,.packages=c("smoothSDE")) %do% {
  #estimation with tensor splines
  formulas <- list(mu1=~1,mu2=~1,tau=~s(ID,bs="re"),
...
xlabel=list("DistanceShore"="Distance to shore","theta"="Theta")
link=list("DistanceShore"=identity,"theta"=identity)
true_smooths=list("omega"=fomega_splines)
...
for (k in 1:N_BATCH) {
  ...
  #names and values of coeffs
  coeff_names=rownames(coeffs)
  coeff_values=as.numeric(coeffs)
  
  #keep coeffs that do not contain ID
  sub_index=grep("ID",coeff_names,invert=TRUE)
  
  #standard deviation of random effects
  sdev=m$sdev()
  sdev_names=rownames(sdev)[1:2]
  sdev_values=as.numeric(sdev)[1:2]
  
  #bind dataframe
  sub_coeffs_df=data.frame("estimate"=c(coeff_values[sub_index],sdev_values),"coeff_name"=factor(c(coeff_names[sub_index],sdev_names)))
  coeffs_df=rbind(coeffs_df,sub_coeffs_df)
}
...
p=ggplotly(p)
saveWidget(p, file ="crcvm_rect_hf_ne2/all_batches_coeffs_crcvm_rect_hf_ne1.html")
...
```{r compute biais and rmse DistanceShore splines}
biais=c()
rmse=c()

#biais and rmse for omega coefficients
for (string in names(sp_coeff_Dshore)) {
  name=paste("omega.",string,sep="")
  true_value=sp_coeff_Dshore[string]
  estimates=coeffs_df[coeffs_df$coeff_name==name,"estimate"]
  biais=c(biais,1/N_BATCH*sum(abs(estimates-true_value)))
  rmse=c(rmse,sqrt(1/N_BATCH*sum((estimates-true_value)^2)))
}


#add biais and rmse for tau
true_log_tau=0
estimates=coeffs_df[coeffs_df$coeff_name=="tau.(Intercept)","estimate"]
biais=c(biais,1/N_BATCH*sum(abs(estimates-true_log_tau)))
rmse=c(rmse,sqrt(1/N_BATCH*sum((estimates-true_log_tau)^2)))

#add biais and rmse for nu
true_log_nu=log(4)
estimates=coeffs_df[coeffs_df$coeff_name=="nu.(Intercept)","estimate"]
biais=c(biais,1/N_BATCH*sum(abs(estimates-true_log_nu)))
rmse=c(rmse,sqrt(1/N_BATCH*sum((estimates-true_log_nu)^2)))

#add biais and rmse for sdev tau
true_sdev_tau=0.1
estimates=coeffs_df[coeffs_df$coeff_name=="tau.(ID)","estimate"]
biais=c(biais,1/N_BATCH*sum(abs(estimates-true_sdev_tau)))
rmse=c(rmse,sqrt(1/N_BATCH*sum((estimates-true_sdev_tau)^2)))

#add biais and rmse for sdev tau
true_sdev_nu=0.3
estimates=coeffs_df[coeffs_df$coeff_name=="nu.(ID)","estimate"]
biais=c(biais,1/N_BATCH*sum(abs(estimates-true_sdev_nu)))
rmse=c(rmse,sqrt(1/N_BATCH*sum((estimates-true_sdev_nu)^2)))


names(biais)=c(paste("omega.",names(sp_coeff_Dshore),sep=""),"tau.(Intercept)","nu.(Intercept)","tau.(ID)","nu.(ID)")
names(rmse)=c(paste("omega.",names(sp_coeff_Dshore),sep=""),"tau.(Intercept)","nu.(Intercept)","tau.(ID)","nu.(ID)")

```

```{r print biais DistanceShore splines}
biais
```

```{r print rmse DistanceShore splines}
rmse
```
...
atw=list("DistanceShore"=fDshore,"AngleNormal"=fangle)
sim_data=crcvm_rect_hf_ne2$BATCH1$simulate(z0=x0[1,],data=crcvm_rect_hf_ne2$BATCH1$data(),atw=atw,land=domain)
...
ggsave("sim_crcvm_rect_hf_ne2.png",plot=plot_sim,width=10,height=5,path="crcvm_rect_hf_ne2")
ggplotly(plot_sim)
```
