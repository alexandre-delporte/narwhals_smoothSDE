...
#used in function split_term to get covariates names from the formulas
library(stringr)
...
plot_samples_rect_hf_ne=ggplot()+geom_sf(data=domain$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_rect_hf_ne[data_rect_hf_ne$ID==1,],mapping=aes(y1,y2),size=0.1)+
  geom_path(data=data_rect_hf_ne[data_rect_hf_ne$ID==1,],mapping=aes(y1,y2),size=0.1)+
...
plot_samples_rect_hf_e=ggplot()+geom_sf(data=domain$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_rect_hf_e[data_rect_hf_e$ID==1,],mapping=aes(y1,y2),size=0.1)+
  geom_path(data=data_rect_hf_e[data_rect_hf_e$ID==1,],mapping=aes(y1,y2),size=0.1)+
...
## Estimate from high frequency data without low error 
...
formulas <- list(mu1=~1,mu2=~1,tau=~1,
                 nu=~1,omega=~te(theta,ExpShore,k=sp_df,bs="cs"))
par0 <- c(0,0,1,4,0)


crcvm_rect_hf_ne1 <- SDE$new(formulas = formulas,data = data_rect_hf_ne[data_rect_hf_ne$ID==1,],type = "RACVM",
...
res=crcvm_rect_hf_ne1$get_all_plots(baseline=NULL,model_name="crcvm_rect_hf_ne1",xmin=xmin,xmax=xmax,
...
p=crcvm_rect_hf_ne1$plot_violin_coeff(model_name="crcvm_rect_hf_ne1",substring="omega",true_coeff=true_coeff,save=TRUE)
...
sim_data=crcvm_rect_hf_ne1$simulate(z0=x0[1,],data=crcvm_rect_hf_ne1$data(),atw=atw,land=domain)
...
crcvm_rect_hf_ne2 <- SDE$new(formulas = formulas,data = data_rect_hf_ne[data_rect_hf_ne$ID==1,],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2","nu","tau"),
                      other_data=list("H"=H0_hf))
...
res=crcvm_rect_hf_ne2$get_all_plots(baseline=NULL,model_name="crcvm_rect_hf_ne2",xmin=xmin,xmax=xmax,
                                    xlabel=xlabel,link=link,show_CI="pointwise",true_smooths=true_smooths,save=TRUE)
...
p=crcvm_rect_hf_ne2$plot_violin_coeff(model_name="crcvm_rect_hf_ne2",substring="omega",true_coeff=true_coeff,save=TRUE)
...
crcvm_rect_hf_e1 <- SDE$new(formulas = formulas,data = data_rect_hf_e[data_rect_hf_e$ID==1,],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2","nu","tau"),
...
link=list("ExpShore"=(\(x) 1/x))
true_smooths=list("omega"=fomega)
res=crcvm_rect_hf_e1$get_all_plots(baseline=NULL,model_name="crcvm_rect_hf_e1",xmin=xmin,xmax=xmax,
                                    xlabel=xlabel,link=link,show_CI="pointwise",true_smooths=true_smooths,save=TRUE)
res$fe_omega_theta_ExpShore
```
...
p=crcvm_rect_hf_e1$plot_violin_coeff(model_name="crcvm_rect_hf_e1",substring="omega",true_coeff=true_coeff,save=TRUE)
...
crcvm_rect_hf_e2 <- SDE$new(formulas = formulas,data = data_rect_hf_e[data_rect_hf_e$ID==1,],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2","nu","tau"),
                      other_data=list("H"=H1_hf))
...
xmin=list("theta"=-pi,"DistanceShore"=0.3)
xmax=list("theta"=pi,"DistanceShore"=2)
res=crcvm_rect_hf_e2$get_all_plots(baseline=NULL,model_name="crcvm_rect_hf_e2",xmin=xmin,xmax=xmax,
                                    xlabel=xlabel,link=link,show_CI="pointwise",true_smooths=true_smooths,save=TRUE)
res$fe_omega_theta_DistanceShore
```
...
p=crcvm_rect_hf_e2$plot_violin_coeff(model_name="crcvm_rect_hf_e2",substring="omega",true_coeff=true_coeff,save=TRUE)
