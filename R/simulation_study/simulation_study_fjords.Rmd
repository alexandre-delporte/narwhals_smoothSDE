...
## Simulation with measurement errors and high frequency 
...
plot_samples_fjords_hf_ne=ggplot()+geom_sf(data=land$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_fjords_hf_ne[data_fjords_hf_ne$ID==2,],mapping=aes(y1,y2),size=0.1)+
  geom_point(data=data_fjords_hf_ne[data_fjords_hf_ne$ID==2,],mapping=aes(p1,p2),size=0.01,col="red",shape=5)+
  geom_path(data=data_fjords_hf_ne[data_fjords_hf_ne$ID==2,],mapping=aes(y1,y2),linewidth=0.1)+
  geom_point(data = data_fjords_hf_ne[data_fjords_hf_ne$ID==2,] %>% filter(!duplicated(ID)),
...
## sImulate with high frequency and measurement error
...
plot_samples_fjords_hf_e=ggplot()+geom_sf(data=land$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_fjords_hf_e[data_fjords_hf_e$ID==2,],mapping=aes(y1,y2),size=0.1)+
   geom_path(data=data_fjords_hf_e[data_fjords_hf_e$ID==2,],mapping=aes(y1,y2),linewidth=0.1)+
  geom_point(data = data_fjords_hf_e[data_fjords_hf_e$ID==2,] %>% filter(!duplicated(ID)),
...
add_covs=function(data) {
  ...
  #this function returns a dataframe with three mùore columns : "theta", "DistanceShore" and "ExpShore"
...
crcvm_fjords_hf_ne1 <- SDE$new(formulas = formulas,data = data_fjords_hf_ne[data_fjords_hf_ne$ID==2,],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2","nu","tau"),
                      other_data=list("H"=H0_hf))
crcvm_fjords_hf_ne1$fit(method="BFGS")
```

#### Plot estimated smooth parameter omega in 3D
...
sim_data=crcvm_fjords_hf_ne1$simulate(z0=x0[1,],data=crcvm_fjords_hf_ne1$data(),atw=atw,land=land)
