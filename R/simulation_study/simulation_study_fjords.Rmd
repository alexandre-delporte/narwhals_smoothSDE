...
ftau_gauss=function(cov_data,D0=0.3,tau0=1,tau1=3) {
  Dshore=cov_data$DistanceShore
  theta=cov_data$theta
...
data_fjords_hf_ne <- foreach(i=1:N_BATCH,.packages=c("progress","MASS","sf","foreach")) %dopar% {
  data=foreach (i=1:N_ID,.combine='rbind') %do% {
  
    res=sim_theta_CRCVM(ftau_constant,fomega,fnu_constant,log_sigma_obs=log(sigma_obs_0),v0,x0[i,],times_hf,land=land,verbose=FALSE)
...
plot_samples_fjords_hf_ne=ggplot()+geom_sf(data=domain$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
...
data_fjords_hf_e <- foreach(i=1:N_BATCH,.packages=c("progress","MASS","sf","foreach")) %dopar% {
  data=foreach (i=1:N_ID,.combine='rbind') %do% {
  
    res=sim_theta_CRCVM(ftau_constant,fomega,fnu_constant,log_sigma_obs=log(sigma_obs_1),v0,x0[i,],times_hf,land=land,verbose=FALSE)
...
names(data_fjords_hf_e)=paste("BATCH",1:N_BATCH,sep="")
```
...
count=0
for (k in 1:length(names(data_fjords_hf_e))) {

  data=data_fjords_hf_e[[paste("BATCH",k,sep="")]]
  for (id in unique(data$ID)) {
    sub_data=data[data$ID==id,]
    if (nrow(sub_data) < n_hf) {
      count=count+1
...
plot_samples_fjords_hf_e=ggplot()+geom_sf(data=domain$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  geom_point(data=data_fjords_hf_e$BATCH1,mapping=aes(y1,y2,color=ID),size=0.1)+
  geom_path(data=data_fjords_hf_e$BATCH1,mapping=aes(y1,y2,color=ID),size=0.1)+
  geom_point(data = data_fjords_hf_e$BATCH1%>% filter(!duplicated(ID)),
             aes(x = y1, y = y2), shape = 3, size = 4, col = "red")+
  xlab("x") + ylab("y")
...
for (k in 1:N_BATCH) {
  ...
  df_e=data_fjords_hf_e[[paste("BATCH",k,sep="")]]
  ...
  data_fjords_hf_e[[paste("BATCH",k,sep="")]]=df_e
}
...
crcvm_fjords_hf_ne1<- foreach(i=1:N_BATCH,.packages=c("smoothSDE")) %dopar% {
...
res=crcvm_fjords_hf_ne1$get_all_plots(baseline=NULL,model_name="crcvm_fjords_hf_ne1",xmin=xmin,xmax=xmax,
...
p=crcvm_fjords_hf_ne1$plot_violin_coeff(model_name="crcvm_fjords_hf_ne1",substring="omega",true_coeff=true_coeff,save=TRUE)
...
crcvm_fjords_hf_ne2 <- SDE$new(formulas = formulas,data = data_rect_hf_ne[data_rect_hf_ne$ID==1,],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2","nu","tau"),
                      other_data=list("H"=H0_hf))
crcvm_fjords_hf_ne2$fit(method="BFGS")
```
...
res=crcvm_fjords_hf_ne2$get_all_plots(baseline=NULL,model_name="crcvm_fjords_hf_ne2",xmin=xmin,xmax=xmax,
                                    xlabel=xlabel,link=link,show_CI="pointwise",true_smooths=true_smooths,save=TRUE)
...
p=crcvm_fjords_hf_ne2$plot_violin_coeff(model_name="crcvm_fjords_hf_ne2",substring="omega",true_coeff=true_coeff,save=TRUE)
...
crcvm_fjords_hf_e1 <- SDE$new(formulas = formulas,data = data_fjords_hf_e[data_fjords_hf_e$ID==1,],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2","nu","tau"),
...
link=list("ExpShore"=(\(x) 1/x))
true_smooths=list("omega"=fomega)
res=crcvm_fjords_hf_e1$get_all_plots(baseline=NULL,model_name="crcvm_fjords_hf_e1",xmin=xmin,xmax=xmax,
                                    xlabel=xlabel,link=link,show_CI="pointwise",true_smooths=true_smooths,save=TRUE)
res$fe_omega_theta_ExpShore
```
...
p=crcvm_fjords_hf_e1$plot_violin_coeff(model_name="crcvm_fjords_hf_e1",substring="omega",true_coeff=true_coeff,save=TRUE)
...
crcvm_fjords_hf_e2 <- SDE$new(formulas = formulas,data = data_fjords_hf_e[data_fjords_hf_e$ID==1,],type = "RACVM",
                      response = c("y1","y2"),par0 = par0,fixpar=c("mu1","mu2","nu","tau"),
                      other_data=list("H"=H1_hf))
crcvm_fjords_hf_e2$fit(method="BFGS")
```
...
xmin=list("theta"=-pi,"DistanceShore"=0.3)
xmax=list("theta"=pi,"DistanceShore"=2)
res=crcvm_fjords_hf_e2$get_all_plots(baseline=NULL,model_name="crcvm_fjords_hf_e2",xmin=xmin,xmax=xmax,
                                    xlabel=xlabel,link=link,show_CI="pointwise",true_smooths=true_smooths,save=TRUE)
res$fe_omega_theta_DistanceShore
```
...
p=crcvm_fjords_hf_e2$plot_violin_coeff(model_name="crcvm_fjords_hf_e2",substring="omega",true_coeff=true_coeff,save=TRUE)
