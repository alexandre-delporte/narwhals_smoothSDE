...
ftau_bump=function(cov_data,epsilon=pi/10,alpha=pi/5,D0=1) {
    Dshore=cov_data$DistanceShore
    theta=cov_data$theta
    tau=(1.5+0.5*(tanh((theta+(pi/2+epsilon))/alpha)-tanh((theta+(pi/2-epsilon))/alpha))+
      0.5*(tanh((theta+(-pi/2+epsilon))/alpha)-tanh((theta+(-pi/2-epsilon))/alpha)))
    return (tau)
}
...
plot_samples_fjords_hf_ne=ggplot()+geom_sf(data=land$geometry,fill=NA)+coord_sf(datum=st_crs(32626))+
  ...
  geom_point(data = data_fjords_hf_ne$BATCH2%>% filter(!duplicated(ID)),
