...
ftau_gauss=function(cov_data,D0=0.5,tau0=1,tau1=3) {
  Dshore=cov_data$DistanceShore
  theta=cov_data$theta
  tau=tau0+(tau1-tau0)*exp(-((theta-pi/2)/(pi/5))^2-(Dshore/D0)^2)+(tau1-tau0)*exp(-((theta+pi/2)/(pi/5))^2-(Dshore/D0)^2)
  return (tau)
}
...
ftau_step=function(cov_data,epsilon=pi/10,alpha=pi/5) {
    Dshore=cov_data$DistanceShore
    theta=cov_data$theta
