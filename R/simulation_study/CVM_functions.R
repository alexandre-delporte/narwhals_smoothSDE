...
#'
#' @param ftau : smooth function with a dataframe argument containing columns
#'  BoundaryDistance and BoundaryAngle that gives the value of tau. We can choose it constant.
#' @param fomega : smooth function with a dataframe argument containing columns
#'  BoundaryDistance and BoundaryAngle, that gives the value of omega
#' @param fnu smooth function with a dataframe argument containing columns
#'  BoundaryDistance and BoundaryAngle, that gives the value of nu
...
#' @param verbose if TRUE, trace computations along the way.
...
#' - "boundary" is a dataframe with columns "p1","p2" that are the coordinates of the nearest points to the shore
#' and columns BoundaryAngle and BoundaryDistance that are the true values used to simulate
#' the next position
...
sim_CRCVM=function(ftau,fomega,fnu,log_sigma_obs=NULL,v0,x0,times,land,verbose=FALSE,save_omega=FALSE) {
  ...
  data_boundary=data.frame("p1"=NA,"p2"=NA,"true_BoundaryDistance"=NA,"true_BoundaryAngle"=NA)
  ...
  for (i in 2:n) {
    ...
    BoundaryDistance=sqrt(normal[1]^2+normal[2]^2)
    
    
    #if point is already on the land, stop simulation
    if (is_in_border(st_point(xmoins),land)) {
      cat("Stopped : process reached land ! \n")
      data_sim$time=times[1:(i-1)]
      if (save_omega) {
        data_sim$omega=c(omegas,NA)
        l=list("sim"=data_sim[1:(i-2),c("y1","y2","time","omega")],"boundary"=data_boundary[2:(i-1),])
      }
      else {
        l=list("sim"=data_sim[1:(i-2),c("y1","y2","time")],"boundary"=data_boundary[2:(i-1),])
      }
      return (l)
    }
    
    # Calculate the signed angle in radians.
    BoundaryAngle=signed_angle(normal,vmoins)
    
    #add to boundary dataframe
    data_boundary=rbind(data_boundary,c(as.numeric(p),BoundaryDistance,BoundaryAngle))
    
    #parameters
    cov_data=data.frame(BoundaryDistance=BoundaryDistance,BoundaryAngle=BoundaryAngle)
    tau=ftau(cov_data)
  ...
  if (save_omega) {
    data_sim$omega=c(omegas,NA)
    l=list("sim"=data_sim[1:(n-1),c("y1","y2","time","omega")],"boundary"=data_boundary[2:n,])
  }
  else {
      l=list("sim"=data_sim[1:(n-1),c("y1","y2","time")],"boundary"=data_boundary[2:n,])
    }
